...
def decrypt_v3(a_key, a_token, a_verbose=False):
    ...
    # Get sha-256 of key
    l_key = codecs.decode(hashlib.sha256(a_key.encode('utf-8')).hexdigest(), "hex_codec")
    ...
    l_decryptor = Cipher(algorithms.AES(l_key),
...
def encrypt_v3(a_key, a_token, a_verbose=False):
    ...
    # Seed rand with time...
    OpenSSL.rand.seed(str(time.time()).encode(encoding='utf-8'))
    ...
    l_encryptor = Cipher(algorithms.AES(l_key),
        modes.GCM(l_iv),
        backend = default_backend()).encryptor()

    # Encrypt the plaintext and get the associated ciphertext.
    # GCM does not require padding.
    l_ciphertext = l_encryptor.update(a_token.encode(encoding='utf-8')) + l_encryptor.finalize()
...
def main(argv):
    ...
    l_arg_parser = argparse.ArgumentParser(description='Generate Random Security Config Post from Template.',
                usage= '%(prog)s',
                epilog= '')
